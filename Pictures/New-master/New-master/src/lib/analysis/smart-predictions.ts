...
export class SmartPredictor {
  ...
  private calculateConfidence(digit: number, factors: { frequency: number; markovProb: number; streakMomentum: number; gapOverdue: number; volatility: number }) {
    const weights = { frequency: 0.30, markovProb: 0.25, streakMomentum: 0.20, gapOverdue: 0.15, volatility: 0.10 }
    const normalized = {
    ...
    const score = normalized.frequency * weights.frequency + normalized.markovProb * weights.markovProb + normalized.streakMomentum * weights.streakMomentum + normalized.gapOverdue * weights.gapOverdue + normalized.volatility * weights.volatility
