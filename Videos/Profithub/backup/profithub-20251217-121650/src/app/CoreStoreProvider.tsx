...
const CoreStoreProvider: React.FC<{ children: React.ReactNode }> = observer(({ children }) => {
    ...
    const { oAuthLogout, retriggerOAuth2Login, isSingleLoggingIn } = useOauth2({
        handleLogout: async () => client.logout(),
        client,
    });
    // Guard to avoid spamming re-login attempts on repeated auth errors
    const reloginAttemptedRef = useRef(false);
    ...
    const handleMessages = useCallback(
        async (res: Record<string, unknown>) => {
            ...
            if (
                ...
                // Soft-handle auth failures: clear local auth state without reloading,
                // and trigger a single OAuth re-login attempt to recover.
                if (!reloginAttemptedRef.current) {
                    await oAuthLogout();
        ...
        [client, oAuthLogout, retriggerOAuth2Login, isSingleLoggingIn]
    );
    ...
    // Reset guard once authorization succeeds, allowing future recovery attempts if needed
    useEffect(() => {
        if (isAuthorized) {
